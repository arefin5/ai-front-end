...
export default function Home() {
  ...
  const { responseData, error, loading, makeRequest } = useLoginReq();
  ...
  const handleSubmite = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    makeRequest({
      url: "http://localhost:4000/api/login", requestData:
      {
        password,
        email,
      }
    ...
    if (error) {
      ...
      setState({
        user: responseData.user, // Assuming responseData.user contains the user object
